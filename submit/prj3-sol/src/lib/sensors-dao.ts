...
export async function makeSensorsDao(
  mongodbUrl: string
): Promise<Errors.Result<SensorsDao>> {
...
export class SensorsDao {
   ...
   *    DB: a database error was encountered.
   */
  async findSensorTypes(
    search: SensorTypeSearch
  ): Promise<Errors.Result<SensorType[]>> {
    try {
      ...
      // const sensorTypes = await this.sensorTypes
  ...
  async findSensorReadings(
    ...
    try {
      ...
      const projection = { _id: 0 };
      const sensorReadings = await this.sensorReadings
        .find(query, { projection })
        .sort("timestamp")
        .skip(index)
        .limit(count)
