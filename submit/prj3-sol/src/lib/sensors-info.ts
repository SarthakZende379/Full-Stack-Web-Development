...
export class SensorsInfo {
  ...
  async addSensor(req: FlatReq): Promise<Errors.Result<Sensor>> {
    ...
    const sensorTypesResult = await this.dao.findSensorTypes({
    ...
    if (sensorTypes.length !== 1) {
      return Errors.errResult(
        `unknown sensor type "${sensor.sensorTypeId}"`,
        "BAD_ID",
        "sensorTypeId"
      );
    }
    ...
    if (!sensor.expected.isSubrange(sensorType.limits)) {
      ...
      return Errors.errResult(
        `expected range [${min1}, ${max1}] of sensor ` +
          `"${sensor.id}" is not within the limits [${min0}, ${max0}] ` +
          `of sensor-type "${sensor.sensorTypeId}"`,
        "BAD_RANGE",
        "min"
      );
    }
  ...
  async addSensorReading(req: FlatReq): Promise<Errors.Result<SensorReading>> {
    ...
    const sensorsResult = await this.dao.findSensors({
    ...
    if (sensors.length !== 1) {
      return Errors.errResult(
        `unknown sensor id "${sensorReading.sensorId}"`,
        "BAD_ID",
        "sensorId"
      );
    }
...
export async function makeSensorsInfo(
  dao: SensorsDao,
  sensorTypes: FlatReq[] = [],
  sensors: FlatReq[] = [],
  ...
  const addResult = await addSensorsInfo(
    sensorTypes,
    sensors,
    sensorReadings,
    sensorsInfo
...
export async function addSensorsInfo(
  sensorTypes: FlatReq[],
  sensors: FlatReq[],
  sensorReadings: FlatReq[],
