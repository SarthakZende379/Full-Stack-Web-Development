 ...
 */
async function findData<T>(url: URL, displayFn: (t: T) => Record<string, string>): Promise<Errors.Result<PagedValues>> {
    let response;
    try {
        response = await fetch(url.toString(), {
            method: "GET",
    ...
    if (jsonData.isOk) {
        const values = [...jsonData.result].map((e) => {
        ...
        return Errors.okResult({ values, next, prev });
    } else {
        return new Errors.ErrResult(jsonData.errors);
    }
}
 ...
 */
function makeSensorTypeDisplay(sensorType: SensorType): Record<string, string> {
    return {
